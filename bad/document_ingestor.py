...
# Optional lightweight PDF extractor fallback (pypdf)
try:
    ...
    def _extract_pdf(self, data: bytes) -> str:
        ...
        if pypdf is not None:
            try:
                ...
                logger.warning("Failed to extract PDF with pypdf: %s", e)
        logger.warning(
            "pdfminer.six is not installed; using fallback PDF extractor. "
            "Results may be poor."
        )
        # naive fallback: attempt to decode bytes directly, but drop if likely garbage
        decoded = data.decode('utf-8', errors='ignore')
        if self._looks_like_garbage(decoded):
            logger.warning("PDF text looked like binary/garbage; returning empty text.")
            return ""
        return decoded

    def _looks_like_garbage(self, text: str) -> bool:
        """Heuristic to detect unreadable text typical of PDF binary decoding.

        Flags as garbage if the proportion of printable letters/spaces is very low
        or if there are long runs without whitespace.
        """
        if not text:
            return True
        total = len(text)
        printable = sum(ch.isprintable() for ch in text)
        letters_spaces = sum(ch.isalpha() or ch.isspace() for ch in text)
        if printable / max(total, 1) < 0.7:
            return True
        if letters_spaces / max(total, 1) < 0.5:
            return True
        # Very long tokens without whitespace often indicate garbage
        import re as _re
        if any(len(tok) > 80 for tok in _re.split(r"\s+", text)):
            return True
        return False
